...
const EditForm=(props)=>{
    ...
    const {close,id,formdata}=props
    const {transaction_name, category, amount, type, date}=formdata
    ...
    const handleUserInput = (event) => {
      ...
      try{
       const response=await fetch("https://bursting-gelding-24.hasura.app/api/rest/update-transaction",options);
    ...
    return(
        <form  className="form" onSubmit={submitFunction}>
           ...
           <div> <h3> update transaction</h3>
            <span>u can update transaction here</span></div>
            ...
            <div>
             ...
             <button type="submit">Update Transcation</button>
            </div>
